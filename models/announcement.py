...
class Announcement(db.Model):
    ...
    require_rsvp = db.Column(db.Boolean, default=False, nullable=False)

    creator = db.relationship("User")
    views = db.relationship("AnnouncementView", backref="announcement", cascade="all, delete-orphan", lazy=True)
    rsvps = db.relationship("AnnouncementRsvp", backref="announcement", cascade="all, delete-orphan", lazy=True)
    ...
    def to_dict(self):
        return {
            ...
            "require_rsvp": self.require_rsvp,
...
class AnnouncementRsvp(db.Model):
    __tablename__ = "announcement_rsvps"
    ...
    __table_args__ = (
        db.UniqueConstraint("announcement_id", "user_id", name="uq_announcement_user_rsvp"),
    )
